...
export function RegisterPage({ onOpenSettings }: { onOpenSettings(): void }) {
  ...
  // - remail：apiBase + API Key + projectId + productId
  const ready = useMemo(() => {
    ...
    if (isRemail) {
      return (
        Boolean(adminAuth) &&
        Number.isInteger(Number(settings.remailProjectId)) &&
        Number(settings.remailProjectId) > 0 &&
        Number.isInteger(Number(settings.remailProductId)) &&
        Number(settings.remailProductId) > 0
      );
    }
