...
/** 临时邮箱后端：cloudflare_temp_email / DuckMail / YYDS / GPTMail / Remail */
export type MailProvider = 'cloudflare' | 'duckmail' | 'yyds' | 'gptmail' | 'remail';
...
export interface AppSettings {
   ...
   * cloudflare（默认）| duckmail | yyds | gptmail | remail
   */
  mailProvider: MailProvider;
  /**
   * Remail 项目 ID。写入 Python config：remail_project_id。
   * Remail 下单接口要求 projectId + productId。
   */
  remailProjectId: number;
  /** Remail 产品 ID。写入 Python config：remail_product_id。 */
  remailProductId: number;
  /** Remail 邮箱后缀池（前端搜索多选）。写入 Python config：remail_email_suffixes。 */
  remailEmailSuffixes: string[];
  /**
...
function hasDomain(s: AppSettings): boolean {
  ...
  // duckmail / yyds / gptmail / remail：域名可选（由 API 分配或 provider 专用字段指定）
  return true;
}
...
export function validateSettings(s: AppSettings): Record<string, string> {
  ...
  try {
    ...
    if (
      ...
      s.registerIntervalMin > 61
    ) {
      errors.registerIntervalMin =
        '注册间隔须在 0～61（0=不等待，1～60=固定分钟，61=随机 25～50 分钟）';
    }
    {
    ...
    if (!String(mail.apiBase ?? '').trim()) errors['mail.apiBase'] = '请填写邮件后端地址';
    {
      ...
      const isRemail = provider === 'remail';
      // duckmail 公共实例可不填 token；CF 匿名模式可不填；yyds/gptmail/remail 需 API Key
      const needAuth =
        isYyds ||
        isGptmail ||
        isRemail ||
    ...
    }
    {
      const provider = String(s.mailProvider || 'cloudflare').toLowerCase();
      if (provider === 'remail') {
        const projectId = Number(s.remailProjectId);
        const productId = Number(s.remailProductId);
        if (!Number.isInteger(projectId) || projectId <= 0) {
          errors.remailProjectId = '请填写 Remail projectId';
        }
        if (!Number.isInteger(productId) || productId <= 0) {
          errors.remailProductId = '请填写 Remail productId';
        }
      }
    }
    if (!hasDomain({ ...s, mail })) {
