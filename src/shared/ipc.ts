...
export interface RemailProductOption {
  id: number;
  type: string;
  status: string;
  codeEnabled: boolean;
...
export interface RemailProjectOption {
  id: number;
  name: string;
  ...
  products: RemailProductOption[];
}

export interface RemailProjectsResult {
  ok: boolean;
  projects: RemailProjectOption[];
  message?: string;
  status?: number;
}
...
export interface RendererApi {
  ...
  listRemailProjects(input: {
    apiBase: string;
    adminAuth: string;
  }): Promise<RemailProjectsResult>;
  /**
