...
app.post('/api/mail/remail/projects', async (req: Request, res: Response) => {
  try {
    const body = (req.body ?? {}) as { apiBase?: string; adminAuth?: string };
    let apiBase = String(body.apiBase || '').trim().replace(/\/+$/, '');
    let apiKey = String(body.adminAuth || '').trim();
    if (apiKey.length >= 7 && apiKey.slice(0, 7).toLowerCase() === 'bearer ') {
      apiKey = apiKey.slice(7).trim();
    }
    if (!apiBase) {
      return res.json({ ok: false, projects: [], message: '请先填写 Remail API 地址' });
    }
    if (!apiKey) {
      return res.json({ ok: false, projects: [], message: '请先填写 Remail API Key' });
    }
    for (const suffix of [
      '/openapi.json',
      '/docs',
    ...
    const listResp = await requestWithProxyFallback(
      `${apiBase}/v1/open/projects?limit=100&scope=visible`,
      {
        method: 'GET',
        headers,
        proxy,
    ...
    if (listResp.status < 200 || listResp.status >= 300) {
      const raw =
        typeof listResp.data === 'string'
          ? listResp.data
          : listResp.data != null
            ? JSON.stringify(listResp.data)
            : '';
      return res.json({
        ok: false,
        projects: [],
        status: listResp.status,
        message: `Remail 项目拉取失败 HTTP ${listResp.status}${raw ? `: ${raw.slice(0, 160)}` : ''}`
      });
    }

    const data = listResp.data as { items?: any[] } | any[] | null;
    let items = Array.isArray(data) ? data : Array.isArray(data?.items) ? data.items : [];

    async function hydrateProject(project: any): Promise<any> {
      if (Array.isArray(project?.products) && project.products.length > 0) return project;
      const id = Number(project?.id);
      if (!Number.isInteger(id) || id <= 0) return project;
      try {
        const detailResp = await requestWithProxyFallback(
          `${apiBase}/v1/open/projects/${id}`,
          {
            method: 'GET',
            headers,
            proxy,
        ...
        if (detailResp.status >= 200 && detailResp.status < 300 && detailResp.data) {
    ...
    items = await Promise.all(items.map((x) => hydrateProject(x)));
    const projects = items
      .map((p: any) => {
        const products = Array.isArray(p?.products) ? p.products : [];
        return {
          id: Number(p?.id) || 0,
          name: String(p?.name || `Project ${p?.id || ''}`).trim(),
          targetPlatform: p?.targetPlatform ? String(p.targetPlatform) : undefined,
          status: p?.status ? String(p.status) : undefined,
          accessType: p?.accessType ? String(p.accessType) : undefined,
          supportsDotAlias: p?.supportsDotAlias === true,
          supportsPlusAlias: p?.supportsPlusAlias === true,
          products: products
            .map((it: any) => ({
              id: Number(it?.id) || 0,
              type: String(it?.type || ''),
              status: String(it?.status || ''),
              codeEnabled: it?.codeEnabled === true,
              purchaseEnabled: it?.purchaseEnabled === true,
              codePrice: it?.codePrice != null ? String(it.codePrice) : undefined,
              totalAvailable: Number.isFinite(Number(it?.totalAvailable))
                ? Number(it.totalAvailable)
                : undefined,
              publicAvailable: Number.isFinite(Number(it?.publicAvailable))
                ? Number(it.publicAvailable)
                : undefined,
              suffixes: Array.isArray(it?.suffixes)
                ? it.suffixes
                    .map((s: any) => ({
                      suffix: String(s?.suffix || '').trim(),
                      totalAvailable: Number.isFinite(Number(s?.totalAvailable))
                        ? Number(s.totalAvailable)
                        : undefined,
                      publicAvailable: Number.isFinite(Number(s?.publicAvailable))
                        ? Number(s.publicAvailable)
                        : undefined
                    }))
                    .filter((s: { suffix: string }) => !!s.suffix)
                : []
            }))
            .filter((it: { id: number }) => it.id > 0)
        };
      })
    ...
    res.json({
      ok: true,
      projects,
      message: `已加载 ${projects.length} 个 Remail 项目`
    });
  } catch (err) {
    res.json({
      ok: false,
      projects: [],
      message: `Remail 项目拉取失败: ${errorMessage(err)}`
    });
  }
});
...
app.post('/api/test/mail', async (req, res) => {
  try {
    ...
    // ---- Remail：Bearer API Key + /v1/open/apikey/profile ----
    if (provider === 'remail') {
      let remailKey = adminAuth;
      if (remailKey.length >= 7 && remailKey.slice(0, 7).toLowerCase() === 'bearer ') {
        remailKey = remailKey.slice(7).trim();
      }
      if (!remailKey) {
        return res.json({ ok: false, message: '请先填写 Remail API Key（rk- 开头）' });
      }
      let base = apiBase;
      for (const suffix of [
        '/openapi.json',
      ...
      const url = `${base}/v1/open/apikey/profile`;
      try {
        const resp = await requestWithProxyFallback(url, {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${remailKey}`,
            Accept: 'application/json'
          },
          proxy,
          timeoutMs: 15000
        ...
        if (resp.status >= 200 && resp.status < 300) {
          const apiKey =
            resp.data && typeof resp.data === 'object'
              ? (resp.data as { apiKey?: { enabled?: boolean; remainingQuota?: number | null } }).apiKey
              : undefined;
          const enabled = apiKey?.enabled !== false;
          return res.json({
            ok: enabled,
            message: enabled
              ? `Remail 连通（API Key 可用${apiKey?.remainingQuota != null ? ` · 剩余额度 ${apiKey.remainingQuota}` : ''}${resp.via === 'proxy' ? ' · 经代理' : ''}）`
              : 'Remail API Key 已连通但当前为禁用状态',
            ms,
            status: resp.status
          });
        }
        if (resp.status === 401 || resp.status === 403) {
          return res.json({
            ok: false,
            message: `已连上 ${base}，但 Remail API Key 被拒（HTTP ${resp.status}）`,
            ms,
            status: resp.status
          });
        }
