...
function applyEnvOverrides(s: AppSettings, source: Partial<AppSettings>): AppSettings {
  ...
  const envRemailProjectId = env.REMAIL_PROJECT_ID ? Number(env.REMAIL_PROJECT_ID) : undefined;
  const envRemailProductId = env.REMAIL_PRODUCT_ID ? Number(env.REMAIL_PRODUCT_ID) : undefined;
  ...
  return {
    ...
    mailProvider:
      source.mailProvider === undefined && env.MAIL_PROVIDER
        ? asMailProvider(env.MAIL_PROVIDER, s.mailProvider)
        : s.mailProvider,
    remailProjectId:
      source.remailProjectId === undefined &&
      Number.isInteger(envRemailProjectId) &&
      (envRemailProjectId as number) > 0
        ? (envRemailProjectId as number)
        : s.remailProjectId,
    remailProductId:
      source.remailProductId === undefined &&
      Number.isInteger(envRemailProductId) &&
      (envRemailProductId as number) > 0
        ? (envRemailProductId as number)
        : s.remailProductId,
    remailEmailSuffixes:
      source.remailEmailSuffixes === undefined && env.REMAIL_EMAIL_SUFFIXES
        ? parseStringList(env.REMAIL_EMAIL_SUFFIXES)
        : s.remailEmailSuffixes,
...
function merge(partial: unknown): AppSettings {
  ...
  const merged: AppSettings = {
    ...
    remailProjectId: (() => {
      const n = Number((p as AppSettings).remailProjectId);
      if (Number.isInteger(n) && n > 0) return n;
      return DEFAULT_SETTINGS.remailProjectId;
    })(),
    remailProductId: (() => {
      const n = Number((p as AppSettings).remailProductId);
      if (Number.isInteger(n) && n > 0) return n;
      return DEFAULT_SETTINGS.remailProductId;
    })(),
    remailEmailSuffixes: Array.isArray((p as AppSettings).remailEmailSuffixes)
      ? Array.from(
          new Set(
            (p as AppSettings).remailEmailSuffixes
              .map((x) => String(x || '').trim().replace(/^@+/, ''))
              .filter(Boolean)
          )
        )
      : DEFAULT_SETTINGS.remailEmailSuffixes,
    mailDomains,
