...
| **邮件后端** | Cloudflare Temp Email / DuckMail / YYDS Mail / GPTMail / Remail |
...
当前根目录 `docker-compose.yml` 默认只挂载 `./data`，生产升级直接拉取镜像即可。旧部署如果还保留 `./register:/opt/register-host:ro`，entrypoint 会 **优先同步宿主 register**，只强拉镜像时 UI/`Build:` 仍可能是旧脚本：
...
| `RUN_COUNT` | 注册轮数默认值 |
| `MAIL_PROVIDER` | 邮件提供方：`cloudflare` / `duckmail` / `yyds` / `gptmail` / `remail` |
| `MAIL_API_BASE` | 邮件后端 API 根地址；Remail 填 `https://remail.aishop6.com` |
| `MAIL_ADMIN_AUTH` | 邮件后端密钥；Cloudflare 为 admin 密码，Remail 为 `rk-...` API Key |
| `MAIL_DOMAIN` | Cloudflare 可收信域名；Remail 可留空 |
| `REMAIL_PROJECT_ID` / `REMAIL_PRODUCT_ID` | Remail 专用；也可启动后在设置页下拉选择 |
| `REMAIL_EMAIL_SUFFIXES` | Remail 后缀多选池，逗号或换行分隔；留空则由 Remail 默认分配 |
...
本地 build 使用 `docker/docker-compose.yml`：
